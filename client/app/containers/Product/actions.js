...
export const filterProducts = (n, v) => {
  return async (dispatch, getState) => {
    try {
      ...
      // Only include specialOffer and newArrivals if they are true
      if (params.specialOffer) {
        params.specialOffer = true;
      } else {
        delete params.specialOffer;
      }
...
export const fetchStoreProduct = slug => {
  return async (dispatch, getState) => {
    // Clear previous product and set loading
    dispatch({
...
export const addProduct = () => {
  return async (dispatch, getState) => {
    try {
      // Set loading state
      dispatch({ type: 'SET_ADDING_PRODUCT', payload: true });
      ...
      // Extract and validate images first
      let images = [];
      if (product.images) {
        // Handle array of File objects
        if (Array.isArray(product.images)) {
          images = product.images.filter(img => img instanceof File);
        } else if (product.images instanceof File) {
      ...
      if (images.length === 0) {
        return dispatch({ 
          ...
          payload: { images: 'Please upload at least one product image (max 5 images).' } 
        });
      }
      ...
      if (images.length > 5) {
        return dispatch({ 
          type: SET_PRODUCT_FORM_ERRORS, 
          payload: { images: 'You can upload a maximum of 5 images per product.' } 
        });
      }
      ...
      // Clear loading state
      dispatch({ type: 'SET_ADDING_PRODUCT', payload: false });
    }
  };
};
  ...
  switch (n) {
    case 'category':
      return {
        ...
        rating: s.rating,
        order: s.order,
        specialOffer: s.specialOffer || false,
        newArrivals: s.newArrivals || false,
        page: s.currentPage,
        limit: s.limit
      };
    case 'brand':

...
exports.caculateTaxAmount = order => {
  try {
    ...
    // Order.total already includes shipping, so we just add tax
    // Calculate subtotal (products only) by subtracting shipping from total
    const shippingCost = order.shippingOption && order.shippingOption.cost 
      ? parseFloat(order.shippingOption.cost) 
      : 0;
    
    // totalWithTax = products + tax + shipping
